...
const activateBtn = (e) => {
    ...
    const selectedAttr = targetBtn.getAttribyte('data-filter');// 클릭 요소 카테고리 문자 저장
    console.log(selectedData, selectedAttr);

/********속성 값을 이용한 필터링 *********/
    
//     filterImg.forEach((image) => {
//         image.classList.add('hide'); // 버튼ㄴ 클릭 시 초기 모든 이미지 요소에 hide 클래스 추가하여 가려줌
//         setTimeout(() => {
...
//         }  else {
//             // selectedAttr 과 imagesAttr 이 다른 요소에 hide 클래스 추가 -> 기존에 있던 show 클래스 삭제
...
/****filter 메서드를 이용한 필터링  ******/
    ...
    // );
        setTimeout(()=> {
            if(
            i.children[0].children[0].getAttribute('src').includes('selectedAttr')||
            selectedAttr ===  'all'
            ) {
                image.classList.remove('hide');
                image.classList.add('show'); 
